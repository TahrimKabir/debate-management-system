   ...
   <body>
     <form  method="post" style="margin-left:20vw;padding-top:15vh;">
       ...
       <?php if (isset($_POST['add'])) {
         ...
         if(empty($mem)){
           ...
           $query = mysqli_query($conn,"INSERT INTO sub_executive(fname,lname,email,phone,dept,image,password)VALUES('$fname',
     ...
     <div class="print">
       <table id="example" class="display nowrap" width="100%">
          <caption style="font-size:7vh; font-weight:bold;">Sub-Executive List </caption>
        <thead>
            <th>SL</th>
            <th>First Name</th>
            <th>Last Name</th>
        ...
        <tbody>
          <?php $s = mysqli_query($conn,"SELECT * FROM sub_executive");
          $i=1;
                     ...
                     $ses ="";
                      if (isset($_POST["delete$i"])) {
                        //$assign_to = $_POST['assign_to'];
                          ...
                          $edit = mysqli_query($conn,"DELETE FROM sub_executive WHERE email='$email'");
                          if ($edit) {
                            echo "<script> window.location.replace('add-sub-executive.php')</script>";
                          }
                      }
